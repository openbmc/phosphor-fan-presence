...
class Group
{
    public:
         ...
         */
        Group(const std::vector<std::string>& names) :
                _names(names)
        {
        }
        ...
        /**
         * Checks if the group's sensors are trusted based
         * on this sensor's newest information.
         *
        ...
        virtual bool checkGroupTrust(
                const monitor::TachSensor& sensor) = 0;
        ...
        auto checkTrust(const monitor::TachSensor& sensor)
        {
            if (inGroup(sensor))
            {
                auto trust = checkGroupTrust(sensor);
