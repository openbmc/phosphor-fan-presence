...
class Zone : public phosphor::fan::monitor::ZoneBase
{
  ...
  private:
     ...
     */
    json captureSensorData() override;
     ...
     *
     */
    void subscribeSensorsToServices() override;
     ...
     */
    const std::vector<CreateGroupFunction> getTrustGroups(
        const json& jsonObj) override;
     ...
     */
    void setTrustMgr(
        const std::vector<CreateGroupFunction>& groupFuncs) override;
     ...
     */
    void updateFanHealth(const Fan& fan) override;
    ...
    void tachSignalOffline(sdbusplus::message_t& msg,
                           const SensorMapType& sensorMap) override;
     ...
     */
    void powerStateChanged(bool powerStateOn) override;
     ...
     * @param[in] jsonObj - JSON object to parse from
     */
    void setFaultConfig(const json& jsonObj) override;

    /**
     * @brief Log an error and shut down due to an offline fan controller
     */
    void handleOfflineFanController() override;
