...
SensorMapType System::buildNameOwnerChangedMap() const
{
    ...
    for (const auto& [sensorName, services] : objects)
    {
                               ...
                               SensorMapType const& sensorMap)
{
    ...
    bool onOffline = !newOwner.empty() && oldOwner.empty();

    std::string stateStr(onOffline ? "online" : "offline");
