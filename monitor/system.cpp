...
bool System::subscribeHwmon()
{
    ...
    try
    {
                                                         ...
                                                         ifacesVec, 0))
        {
            ...
            {
                auto wasAdded = svcNames.insert(service.first);

                // register the match callback for each unique service
                if (wasAdded.second)
                {
                    ...
                    try
                    {
                        ...
                        _hwmonMatches.push_back(std::make_unique<match::match>(
                            _bus, match::rules::nameOwnerChanged(service.first),
                            std::bind(&System::hwmonCallback, this)));
                    }
                    catch (std::exception& e)
                    {
                        getLogger().log(
                            fmt::format("Caught D-Bus exception during "
                                        "callMethodAndRead(): {}",
                                        e.what(), Logger::error));
                        svcRunning = false;
                    }
    ...
    if (fanPaths.size() > 0 && !serviceError)
    {
    ...
    return !serviceError && (_hwmonMatches.size() == svcNames.size());
}
