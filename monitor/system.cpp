...
void System::start()
{
    ...
    for (auto const& service_itr : sensorMap)
    {
        try
        {
            _sensorMatch.push_back(
                std::make_unique<sdbusplus::bus::match::match>(
                    _bus, rules::nameOwnerChanged(service_itr.first),
...
SensorMapType System::buildNameOwnerChangedMap() const
{
    ...
    auto objects = util::SDBusPlus::getSubTree(_bus, FAN_SENSOR_PATH,
                                               FAN_SENSOR_VALUE_INTF, 0);
    ...
    for (const auto& fan : _fans)
    {
        ...
        for (const auto& sensor : fan->sensors())
        {
            ...
            if (objects.end() == itServ || itServ->second.empty())
            {
                getLogger().log(
            ...
            const auto& serviceName = itServ->second.begin()->first;
                               ...
                               SensorMapType const& sensorMap)
{
    ...
    getLogger().log(fmt::format("Changing sensors for service {} to {}",
                                serviceName.c_str(), stateStr.c_str()),
                    Logger::info);
