...
TachSensor::TachSensor(Mode mode, sdbusplus::bus::bus& bus, Fan& fan,
                       ...
                       const std::string& interface, size_t factor,
                       size_t offset, size_t timeout,
                                    ...
                                    const std::string& propertyName, T& value)
{
    ...
    if (sensor.compare(interface) == 0)
    {
        ...
        {
            value = sdbusplus::message::variant_ns::get<T>(propertyMap->second);
        }
    }
}
...
void TachSensor::startTimer(TimerMode mode)
{
    if (!timerRunning())
    {
        _timer.start(getDelay(mode), util::Timer::TimerType::oneshot);
        _timerMode = mode;
    }
    else
    {
