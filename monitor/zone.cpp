...
const std::vector<CreateGroupFunction> Zone::getTrustGroups(const json& jsonObj)
{
    return phosphor::fan::monitor::getTrustGrps(jsonObj);
}

void Zone::setTrustMgr(const std::vector<CreateGroupFunction>& groupFuncs)
{
...
void Zone::setFaultConfig(const json& jsonObj)
{
    ...
    PowerOffAction::PrePowerOffFunc func =
        std::bind(std::mem_fn(&Zone::logShutdownError), this);
                ...
                const std::vector<FanTypeDefinition>& fanDefs)
{
    ...
    try
    {
        ...
        lg2::info("Configuration loaded for Zone {ZONE_NAME}", "ZONE_NAME",
                  _name);
...
#endif
        bus, std::bind(std::mem_fn(&Zone::powerStateChanged), this,
                       std::placeholders::_1))),
