        ...
        _sensors.emplace_back(
            std::make_unique<TachSensor>(bus,
                                         *this,
                                         ...
                                         std::get<startupTimeoutField>(def),
...
void Fan::tachChanged(TachSensor& sensor)
{
    ...
    //If this sensor is too slow at this moment, start
    ...
    else
    {
        ...
        if ((_functional != functionalState::FUNCTIONAL) &&
            !tooManySensorsNonfunctional())
        {
            if (_functional != functionalState::UNKNOWN)
            {
...
bool Fan::tooSlow(const TachSensor& sensor)
{
    uint64_t actual = sensor.getInput();
    uint64_t target = getTargetSpeed(sensor);

    if (actual < (target * _deviation) / 100)
    {
        return true;
    }
