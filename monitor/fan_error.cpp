...
void FanError::commit(const json& jsonFFDC, bool isPowerOffError)
{
    ...
    auto service_fd = mkstemp(tmpFile);
    if (service_fd != -1)
    {
        auto ffdcData = getJournalEntries(25);
        const auto& jsonString = ffdcData.dump();
        auto rc = write(service_fd, jsonString.data(), jsonString.size());
        if (rc != -1)
        {
            ffdc.emplace_back(FFDCFormat::JSON, 0x01, 0x01, service_fd);
        }
    }
