...
nlohmann::json FanError::getJournalEntries(int numLines) const
{
    ...
    if (rc < 0)
    {
        ...
        entries.push_back("[Internal error: sd_journal_open()]");
        return json(entries);
    }
    ...
    for (const auto& executable : executables)
    {
        ...
        {
            // Build one line string containing field values
            entries.push_back("[Internal error: sd_journal_add_match()]");
...
std::string FanError::getTimeStamp(sd_journal* journal) const
{
    ...
    if (rc < 0)
    {
        return "[Internal error: sd_journal_get_realtime_usec()]";
    }
    ...
    {
        return "[Internal error: localtime()]";
    }
