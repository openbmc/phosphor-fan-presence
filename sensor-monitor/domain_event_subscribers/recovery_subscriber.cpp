...
RecoverySubscriber::RecoverySubscriber(DbusAlarmMonitor& dbusAlarmMonitor) :
    dbusAlarmMonitor(dbusAlarmMonitor)
{
...
void RecoverySubscriber::handle(DomainEvent& event)
{
    if (typeid(event) == typeid(SystemProtectionTriggered))
    {
        ...
        {
            dbusAlarmMonitor.watchSensorAlarm(sensorPath, recoveryType);
        }
    }
    else if (typeid(event) == typeid(SystemRecoveryTriggered))
    {
        auto& theEvent = dynamic_cast<SystemRecoveryTriggered&>(event);
        dbusAlarmMonitor.stopWatchAlarm(theEvent.getSensorPath(),
                                        theEvent.getAlarmType());
    }
}

std::vector<AlarmType>
    RecoverySubscriber::obtainRecoveryTypesOf(const std::string& sensorPath)
{
    std::vector<AlarmType> result;

    auto it = sensorsRecoveryThresholds.find(sensorPath);
    if (it == sensorsRecoveryThresholds.end())
    {
        return result;
    }

    for (auto& threshold : it->second)
    {
        ...
        {
            log<level::ERR>(
                fmt::format("{} is not match to any threshold interface",
                            threshold)
                    .c_str());
        }
    }
...
void RecoverySubscriber::parse(json& recoveryConfig)
{
    for (auto& sensorConfig : recoveryConfig.at("sensors"))
    {
        try
        {
            ...
            {
                thresholds.emplace_back(threshold.at("alarm"));
            }

            sensorsRecoveryThresholds.emplace(sensorPath, thresholds);
        }
...
void RecoverySubscriber::loadRecoveryActionConfig()
{
    ...
    {
        log<level::ERR>(fmt::format("Config file: {}/{} doesn't exist",
