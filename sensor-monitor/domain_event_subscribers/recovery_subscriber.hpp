  ...
  private:
    std::vector<AlarmType> obtainRecoveryTypesOf(std::string sensorPath)
    {
      ...
      for(auto& [threshold, delay]: it->second)
      {
        ...
        else if (threshold == "WarningAlarmLow" || threshold == "WarningAlarmHigh")
        {
        ...
        else
        {
          log<level::ERR>(fmt::format("{} is not match to any\
              threshold interface", threshold).c_str());
        }
      }
    ...
    void parse(json& recoveryConfig)
    {
      for (auto& sensorConfig: recoveryConfig.at("sensors"))
      {
        ...
        {
          log<level::ERR>(fmt::format("Failed to parse config file: {}", e.what()).c_str());
        }
      }
    }
    ...
    void loadRecoveryActionConfig()
    {
      ...
      if (!std::filesystem::exists(path))
      {
        log<level::ERR>(fmt::format("Config file: {}/{} don't exist",
          SENSOR_MONITOR_PERSIST_ROOT_PATH, recoveryConfigName).c_str());
        return;
      }
    ...
    std::map<std::string, std::map<std::string, int>> _sensorsRecoveryThresholds;
