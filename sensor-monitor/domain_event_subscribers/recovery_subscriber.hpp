...
class RecoverySubscriber : public DomainEventSubscriber
{
  public:
     ...
     */
    explicit RecoverySubscriber(DbusAlarmMonitor& dbusAlarmMonitor);
  ...
  private:
     ...
     */
    std::vector<AlarmType> obtainRecoveryTypesOf(const std::string& sensorPath);
     ...
     * recovery config
     */
    std::map<std::string, std::vector<std::string>> sensorsRecoveryThresholds;
