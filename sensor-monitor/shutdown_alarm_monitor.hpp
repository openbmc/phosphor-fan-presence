 ...
 * be shut down.  The timer values are can be modified with
 * configure.ac options.  If the alarm is cleared before the timer
 * expires, then the timer is stopped.
 *
 * Event logs are also created when the alarms trip and clear.
 *
 * Note that the SoftShutdown alarm actually implements a hard shutdown,
 * because in the system implementation this is being targeted for the
 * host itself will handle the soft shutdown, and if it isn't done by
 * the time the shutdown timer expires then a hard shutdown is required.
 * This behavior could be modified with compile flags if anyone needs
 * a different behavior in the future.
 *
