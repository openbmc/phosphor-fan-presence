...
void ProtectionAlarmHandler::startTimer(const AlarmKey& alarmKey)
{
    ...
    {
        log<level::ERR>(
            fmt::format("Failed to obtain sensor's value: {}", e.what())
                .c_str());
    }
    ...
    {
        auto original = previousStartTime->second;
...
bool ProtectionAlarmHandler::isBackToNormal(bool isAlarmTriggered,
                                            bool hasTimerBeenStarted)
{
    return !isAlarmTriggered && hasTimerBeenStarted;
}
                                   ...
                                   const AlarmKey& alarmKey)
{
    ...
    auto hasTimerBeenStarted = alarm->second ? true : false;
    if (isAlarmTriggered && !hasTimerBeenStarted)
    {
        startTimer(alarmKey);
    }
    else if (isBackToNormal(isAlarmTriggered, hasTimerBeenStarted))
    {
        stopTimer(alarmKey);
    }
}
    ...
    std::map<std::string, std::variant<bool>> properties)
{
    ...
    const auto& lowAlarmName =
        alarmProperties.at(handledType).at(AlarmDirection::low);
    if (properties.contains(lowAlarmName))
    {
        AlarmKey alarmKey{sensorPath, handledType, AlarmDirection::low};
        auto alarm = alarms.find(alarmKey);
        if (alarm == alarms.end())
        {
    ...
    const auto& highAlarmName =
        alarmProperties.at(handledType).at(AlarmDirection::high);
    if (properties.contains(highAlarmName))
    {
        AlarmKey alarmKey{sensorPath, handledType, AlarmDirection::high};
        auto alarm = alarms.find(alarmKey);
        if (alarm == alarms.end())
        {
            alarms.emplace(alarmKey, nullptr);
...
bool RecoveryAlarmHandler::isBackToAbnormal(bool isAlarmTriggered,
                                            bool hasTimerBeenStarted)
{
                                 ...
                                 const AlarmKey& alarmKey, uint16_t timerDelay)
{
    ...
    else if (isBackToAbnormal(isAlarmTriggered, hasTimerBeenStarted))
    {
        stopTimer(alarmKey);
    }
}
        ...
        for (auto sensorConfig : recoveryConfig.at("sensors"))
        {
            ...
            if (theSensorPath == sensorPath)
            {
                ...
                std::map<AlarmDirection, std::string> searchingProperties =
                    alarmProperties.find(handledType)->second;
    ...
    std::map<std::string, std::variant<bool>> properties)
{
    ...
    for (auto& [direction, delay] : obtainTimerConfigFor(sensorPath))
    {
        const auto& alarmPropertyName =
            alarmProperties.at(handledType).at(direction);
        if (properties.contains(alarmPropertyName))
        {
            AlarmKey alarmKey{sensorPath, handledType, direction};
            auto alarm = alarms.find(alarmKey);
            if (alarm == alarms.end())
            {
                alarms.emplace(alarmKey, nullptr);
