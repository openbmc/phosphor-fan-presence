    ...
    _bus(bus), _event(event)
{
    ...
    auto registerSigHandler = [this]() {
        stdplus::signal::block(SIGHUP);

        _signalObj = std::make_unique<sdeventplus::source::Signal>(
            _event, SIGHUP, [this](auto& sigSrc, const auto& sigInfo) {
                _reporter.reset();
    ...
    else
    {
        ...
        _inventoryMatch = std::make_unique<sdbusplus::bus::match::match>(
            ...
            [this, &registerSigHandler](auto& msg) {
                std::string msgStr;
                msg.read(msgStr);
...
{
    _config = std::make_unique<phosphor::fan::JsonConfig>([this]() {
