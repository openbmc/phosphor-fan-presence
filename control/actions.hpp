        ...
        std::map<int64_t, uint64_t>&& val_to_speed)
{
    return [val_to_speed = std::move(val_to_speed)](auto& zone, auto& group)
    {
        ...
        if (group.size() != 0)
        {
            ...
            auto prevValue = zone.getPrevCeilingKey(avgValue);
 ...
 * @details Provides the ability to determine what the net decrease delta the
 * zone's fan speeds should be updated by from their current target speed and
            ...
            state = std::forward<T>(state)](auto& zone, auto& group)
    {
        ...
        std::for_each(
            ...
            [&zone, &state, &speedDelta, &netDelta](auto const& entry)
            {
                ...
                // TODO Properly support possible state types (future?)
                if (value < state)
                {// Net decrease is the difference times the given speed delta
