...
PropertyVariantType
    getDefaultValue(const PropertyVariantType& val,
                    const std::optional<PropertyVariantType>& default_value)
{
...
struct LessThanOperator : public Modifier::BaseOperator
{
    LessThanOperator(const json& jsonObj)
    {
        ...
        {
            default_value = ConfigBase::getJsonValue(jsonObj["default_value"]);
        }
    }
    ...
    PropertyVariantType operator()(double val) override
    {
        ...
        return getDefaultValue(rangeValues.back().second, default_value);
    }
    ...
    std::vector<std::pair<PropertyVariantType, PropertyVariantType>>
        rangeValues;
    std::optional<PropertyVariantType> default_value;
};
