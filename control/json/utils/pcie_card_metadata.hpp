 ...
 *   "sensor_name": "Card_A_\d+_Temp"
 *
 * The sensor_name field above is a regulator expression that can be
 * used to match on its D-Bus temperature sensor names if necessary.
 *
 * The default_floor_index field can be used by the action when
 * the lookup doesn't find a match.
 */
class PCIeCardMetadata
{
  public:
     ...
     * @brief Constructor
     *
     * @param[in] baseConfigFile - The first config file to load
     * @param[in] systemNames - The IBMCompatible names values
     */
     ...
     *
     * @return optional<variant<int32, string>> -
     *     Either the floor index for that entry, or the name of the
     *     D-Bus temp sensor it will expose if found.
     *
    ...
    /**
     * @brief Returns the default floor index in the JSON
     */
    int32_t getDefaultFloorIndex() const
    {
        return _defaultFloorIndex;
    }
  ...
  private:
     ...
     *
     * If baseConfigFile isn't empty, it will load in that file
     *
    ...
    /**
     * @brief The default floor index for when the lookup fails
     */
    int32_t _defaultFloorIndex{0};
};
