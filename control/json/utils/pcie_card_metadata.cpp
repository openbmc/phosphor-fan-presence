                                 ...
                                 const std::vector<std::string>& systemNames)
{
    if (!baseConfigFile.empty())
    {
        ...
        log<level::DEBUG>(
            fmt::format("Loading base PCIe card file {}", confFile.native())
                .c_str());
...
void PCIeCardMetadata::load(const nlohmann::json& json)
{
    ...
    for (const auto& card : json.at("cards"))
    {
        ...
        if (!data.hasTempSensor && !card.contains("floor_index"))
        {
            throw std::runtime_error{
                "Invalid PCIe card json: missing floor_index"};
        }

        data.floorIndex = card.value("floor_index", 0);

        data.sensorNameRegex = card.value("sensor_name", "");

        if (data.hasTempSensor && data.sensorNameRegex.empty())
        {
            throw std::runtime_error{
                "Missing 'sensor_name' field in PCIe card JSON"};
        }
        ...
        if (iter != _cards.end())
        {
            ...
            iter->sensorNameRegex = data.sensorNameRegex;
        }
        else
        {
    ...
    {
        _defaultFloorIndex = json.at("default_floor_index").get<int32_t>();
    }
}
...
void PCIeCardMetadata::dump() const
{
    ...
    for (const auto& entry : _cards)
    {
        ...
        std::cerr << "sensorNameRegex: " << entry.sensorNameRegex << "\n";
    }
}
