...
class ChassisManager
{
  public:
     ...
     *            targetInterface) when a previously-absent fan sensor's target
     *            interface appears on D-Bus.
     ...
     * handleChassisStatusChange() retries the bind.  The watch removes itself
    ...
    /**
     * @brief Discard all fan-sensor-loss watches for the given chassis.
     *
     * Called when a chassis goes unavailable (all fans removed) or after a
     * full re-bind cycle clears out stale watches.
     *
     * @param[in] chassisPath - Chassis inventory path
     */
  ...
  private:
    ...
    /**
     * Fan sensor hotplug watches: chassisPath -> sensorPath -> match.
     * Each match fires _onFanSensorAppeared when its target interface appears
     * on D-Bus.
     */
    std::map<std::string,
