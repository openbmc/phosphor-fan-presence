 ...
 *   _chassisMgr->init(bus, [this](const auto& p){ handleChassisStatusChange(p);
...
class ChassisManager
{
  public:
     ...
     * objects, and stores the bus reference and status-change callback.
     ...
     *
     * Used by Manager::load() to determine single-chassis vs multi-chassis
     * mode.
     */
    bool empty() const
    {
     ...
     *        D-Bus and re-trigger the chassis callback when it does.
     *
     * Called by Manager::handleChassisStatusChange() for every fan sensor that
     * failed its initial service lookup (hasSensorsOnDbus() == false) while
     * the chassis was already ready.  When the InterfacesAdded signal arrives
     * the stored callback is fired with @p chassisPath so that
     * handleChassisStatusChange() retries the bind.  The watch is one-shot: it
     * removes itself after the first successful signal.
     *
     ...
     */
    void watchFanSensor(const std::string& chassisPath,
                        const std::string& sensorPath,
                        const std::string& targetInterface);

    /**
     * @brief Discard all pending fan-sensor watches for the given chassis.
     *
     * Called after a successful full bind cycle for @p chassisPath so that
     * stale InterfacesAdded matches are not left around.
     *
     * @param[in] chassisPath - Chassis inventory path
     */
    void clearFanSensorWatches(const std::string& chassisPath);
  ...
  private:
    ...
    /**
     * Fan sensor hotplug watches: chassisPath → list of (sensorPath, match).
     * Each entry is a one-shot InterfacesAdded subscription that fires the
     * chassis callback when its target interface appears on D-Bus.
     */
    std::map<
        std::string,
        std::vector<std::pair<std::string, std::unique_ptr<sdbusplus::match>>>>
        _fanSensorWatches;
};
