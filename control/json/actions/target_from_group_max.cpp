...
std::map<uint16_t, uint64_t> TargetFromGroupMax::_speedFromGroupsMap;
...
void TargetFromGroupMax::run(Zone& zone)
{
    ...
    // Flag to check if the groups's value are valid for further processing
    ...
    if (processFlag)
    {
        ...
        uint64_t groupValue =
            static_cast<uint64_t>(std::get<double>(max.value()));
...
void TargetFromGroupMax::setIndex(const json& jsonObj)
{
    if (!jsonObj.contains("index"))
    {
        throw ActionParseError{
            ActionBase::getName(),
            "Missing required neg_hysteresis or index value"};
    }
    _groupIndex = jsonObj["index"].get<uint16_t>();

    if (_speedFromGroupsMap.find(_groupIndex) != _speedFromGroupsMap.end())
    {
        throw ActionParseError{ActionBase::getName(),
                               "The index has been used before"};
    }
    else
    {
        // Initialize the table of groups and their max values
