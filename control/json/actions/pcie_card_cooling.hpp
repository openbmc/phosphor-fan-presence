...
/**
 * @class PCIeCardCooling - Action to set the PCIe card floor index parameter
 ...
 *  - If a card isn't recognized, the value configured as the default
 *    index is used.
 ...
 *    {
 *       "base_pcie_card_file": <filename>
 *       "use_config_specific_files": <true/false>
 *    }
 *
 *    base_pcie_card_file:
 *      - Used to specify a filename that doesn't depend on the system type.
 *      - It can be omitted if use_config_specific_files is true.
 *
    ...
    public ActionRegister<PCIeCardCooling>
{
  ...
  private:
     ...
     *  - The floor index or temp sensor name if found,
     *    std::nullopt else.
     */
