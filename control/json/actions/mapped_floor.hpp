 ...
 *  - If any of the D-Bus properties the groups point at have no owner.
 *    (are not on D-Bus), the code will wait 1 second and try again, once.
 *     - This gives the underlying services a chance to restart without
 *       causing a floor change.
 */
...
class MappedFloor : public ActionBase, public ActionRegister<MappedFloor>
{
  ...
  private:
    ...
    /**
     * @brief Determines if there is a delay in progress due to
     *        a missing owner of a D-Bus group property.
     *
     * Will only do a single 1s delay for a missing owner.
     *
     * @param[in] manager - The Manager object
     *
     * @return bool - If there is a delay in progress so that
     *                run() doesn't need to execute.
     */
    bool groupOwnerDelayCheck(const Manager& manager);

    /**
     * @brief Called by the missing owner timer after 1 second
     *
     * Calls run().
     */
    void ownerTimerExpired();
    ...
    /* @brief The timer used for the missing owner delay */
    sdeventplus::utility::Timer<sdeventplus::ClockId::Monotonic> _ownerTimer;

    /* The current group members with missing owners, concatenated */
    std::string _missingOwners;

    /* Pointer to the zone used in run() */
    Zone* _zone{nullptr};
};
