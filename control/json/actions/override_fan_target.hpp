...
of
 * false means Functional should go to false to be counted. The signal is
declared
    ...
    public ActionRegister<OverrideFanTarget>
{
  ...
  private:
    /* logical state members must each to trigger action*/
    PropertyVariantType _state;

    /* count of group members required to be at _state to trigger action*/
    size_t _count;
