 ...
 */
#include "net_speed_decrease.hpp"
...
NetSpeedDecrease::NetSpeedDecrease(const json& jsonObj) : ActionBase(jsonObj)
{
    setState(jsonObj);
    setDelta(jsonObj);
}

void NetSpeedDecrease::run(Zone& zone, const Group& group)
{
            ...
            else
            {
                ...
                log<level::ERR>(
                    fmt::format(
                        ...
                        "given. [object = {} : {} : {}]",
                        ActionBase::getName(), std::get<pathPos>(member),
                        std::get<intfPos>(member), std::get<propPos>(member))
                        .c_str());
            }
        }
...
void NetSpeedDecrease::setState(const json& jsonObj)
{
...
void NetSpeedDecrease::setDelta(const json& jsonObj)
{
