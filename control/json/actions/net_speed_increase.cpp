 ...
 */
#include "net_speed_increase.hpp"
...
NetSpeedIncrease::NetSpeedIncrease(const json& jsonObj) : ActionBase(jsonObj)
{
    setState(jsonObj);
    setDelta(jsonObj);
}

void NetSpeedIncrease::run(Zone& zone, const Group& group)
{
    auto netDelta = zone.getIncSpeedDelta();
    std::for_each(
        group.begin(), group.end(),
        [this, &zone, &netDelta](const auto& member) {
            try
            {
                auto value = zone.getPropValueVariant(
...
void NetSpeedIncrease::setState(const json& jsonObj)
{
...
void NetSpeedIncrease::setDelta(const json& jsonObj)
{
