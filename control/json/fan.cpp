...
void Fan::setSensors()
{
    ...
    for (const auto& sensorName : _sensorNames)
    {
        if (_targetPath.empty())
        {
            ...
            // it defaults to /xyz/openbmc_project/sensors/fan_tach/
            path = FAN_SENSOR_PATH + sensorName;
        }
        else
        {
            path = _targetPath + sensorName;
        }

        try
        {
            std::string service =
                util::SDBusPlus::getService(_bus, path, _interface);
    ...
    if (!path.empty())
    {
        ...
        {
            lg2::debug("Fan {NAME}: lost service for {PATH} between lookup and "
                       "target read, will retry on hotplug",
