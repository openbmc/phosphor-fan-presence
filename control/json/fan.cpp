...
void Fan::setSensors(const json& jsonObj)
{
    ...
    // If this fan is associated with a chassis, check whether the
    // chassis is ready (present, and available if so configured) before
    // attempting any D-Bus sensor lookups.
    // For existing systems with no "chassis_path" in JSON, _chassisPath is
    // empty and isReady() returns true immediately (backwards compatibility).
    ...
    // Single, non-blocking service lookup per sensor.  If any service is
    // missing (e.g. the fan was hotplugged after chassis-ready), record that
    // path in _pendingSensorPath so Manager::handleChassisStatusChange() can
    // install one InterfacesAdded watch via ChassisManager::watchFanSensor().
    // When the watch fires, handleChassisStatusChange() calls getConfig<Fan>()
    // from scratch, re-attempting ALL sensor lookups — so a single watch on
    // the first failing path is enough to trigger the full retry.
    std::string path;
