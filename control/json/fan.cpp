...
json Fan::dump() const
{
    ...
    // Read feedback for all bound sensors, then attempt to read a companion
    // read-only rotor (name with "_0" replaced by "_1") if one exists -
    // e.g. the ekra2l1 rotor B tach.
    ...
    for (const auto& [path, service] : _sensors)
    {
        ...
        if (name.size() >= 2 && name.substr(name.size() - 2) == "_0")
        {
            ...
            auto companionPath = path.substr(0, path.size() - 2) + "_1";
            try
            {
                auto companionService = util::SDBusPlus::getService(
                    _bus, companionPath, FAN_SENSOR_VALUE_INTF);
                auto value = util::SDBusPlus::getProperty<double>(
                    _bus, companionService, companionPath,
