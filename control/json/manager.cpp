...
{
    // Always construct a fresh ChassisManager so Fan constructors always
    // receive a valid reference.  registerChassis() is only called when the
    // config actually has chassis_path entries.
    ...
    _chassisMgr->init(
        ...
        [this](const std::string& chassisPath, const std::string& sensorPath) {
            handleFanSensorAppeared(chassisPath, sensorPath);
        },
        [this](const std::string& chassisPath, const std::string& sensorPath,
               const std::string& targetInterface) {
            handleFanSensorLost(chassisPath, sensorPath, targetInterface);
        });

    // Scan fans.json to collect unique chassis paths and build the
    // chassis->zones map.  ChassisManager must be initialised before
    // getConfig<Fan>() so Fan::setSensors() sees real D-Bus chassis state.
    auto confFile =
        fan::JsonConfig::getConfFile(confAppName, Fan::confFileName, false);
    if (!confFile.empty())
    {
        ...
        for (const auto& entry : fan::JsonConfig::load(confFile))
        {
            ...
            {
                std::string path = entry["chassis_path"].get<std::string>();
                bool checkAvail =
...
void Manager::handleChassisStatusChange(const std::string& chassisPath)
{
    ...
    for (auto& fan : fans)
    {
        ...
        if (!fan.second->hasSensorsOnDbus())
        {
            ...
            if (!sensorPath.empty())
            {
                _chassisMgr->watchFanSensor(chassisPath, sensorPath,
                                            fan.second->getInterface());
                                      ...
                                      const std::string& sensorPath)
{
    ...
    // Re-construct fans for this chassis so the newly-appeared sensor gets
    // its service resolved.  Only the fan whose sensorPath matches will now
    // have hasSensorsOnDbus() == true.
    auto fans = getConfig<Fan>(false, *_chassisMgr);
    ...
    // Clear the now-satisfied hotplug watch and install loss watches.
    _chassisMgr->clearFanSensorWatches(chassisPath);
                                  ...
                                  const std::string& targetInterface)
{
    ...
    // Discard the now-stale loss watch
    _chassisMgr->clearFanSensorLossWatches(chassisPath);
