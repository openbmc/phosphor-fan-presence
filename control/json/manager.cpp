...
void Manager::handleChassisStatusChange(const std::string& chassisPath)
{
    ...
    // Parse all fans from JSON (no D-Bus I/O yet), then resolve sensor
    // services.  For fans belonging to this chassis, initSensors() will
    // attempt service lookups now that the chassis is ready.  Fans whose
    // sensor service is still absent will have hasSensorsOnDbus() == false
    // and getPendingSensorPath() non-empty.
    auto fans = getConfig<Fan>(false, *_chassisMgr);
    for (auto& [key, fan] : fans)
    {
        fan->initSensors();
                                      ...
                                      const std::string& sensorPath)
{
    ...
    // Parse all fans from JSON (no D-Bus I/O yet), then resolve sensor
    // services.  Only the fan whose sensorPath matches will now have
    // hasSensorsOnDbus() == true.
    auto fans = getConfig<Fan>(false, *_chassisMgr);
    for (auto& [key, fan] : fans)
    {
        fan->initSensors();
    ...
    // Clear only the appear-watch for this specific sensor — other fans on
    // the same chassis may still have pending appear-watches of their own.
