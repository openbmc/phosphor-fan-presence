    ...
    if (_loadAllowed)
    {
        ...
        for (auto& fan : fans)
        {
            if (!fan.second->hasSensorsOnDbus())
            {
                ...
                // appeared on D-Bus.  Install a watch so the fan is bound
                // when the service appears.
                if (_chassisMgr)
                {
                    _chassisMgr->watchFanSensor(
...
void Manager::handleChassisStatusChange(const std::string& chassisPath)
{
    ...
    for (auto& fan : fans)
    {
        ...
        if (!fan.second->hasSensorsOnDbus())
        {
            // Sensor service not yet on D-Bus; install a watch so the fan
            // is bound automatically when the service appears.
