            ...
            if (!confFile.empty())
            {
                ...
                for (const auto& entry : fan::JsonConfig::load(confFile))
                {
                    if (entry.contains("chassis_path"))
                    {
                        ...
                        // Record the zone name for this chassis, avoiding
                        // duplicates (multiple fans share the same zone).
                        if (entry.contains("zone"))
                        {
                            auto& zoneVec = _chassisPathToZones[path];
                            std::string zoneName =
                                entry["zone"].get<std::string>();
                            if (std::find(zoneVec.begin(), zoneVec.end(),
                                          zoneName) == zoneVec.end())
                            {
                                zoneVec.push_back(std::move(zoneName));
                            }
                        }
                    }
                }
