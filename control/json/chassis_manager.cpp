        ...
        *_bus, sdbusplus::match_rules::interfacesAddedAtPath(sensorPath),
        [this, chassisPath, sensorPath,
         targetInterface](sdbusplus::message_t& msg) {
            ...
            // Do NOT erase the match here — we are currently executing inside
            // the match object's own lambda.  Erasing the unique_ptr now would
            // destroy the match object mid-callback, causing a crash on return.
            // clearFanSensorWatches() called at the end of
            // handleChassisStatusChange() will clean up all watches for this
            // chassis once all fans are bound.
            if (_onChassisStatusChanged)
            {
                                        ...
                                        const std::string& targetInterface)
{
    ...
    lg2::debug("ChassisManager: watching for loss of fan sensor {SENSOR} "
               "interface {INTF} (chassis {PATH})",
               "SENSOR", sensorPath, "INTF", targetInterface, "PATH",
               chassisPath);
        ...
        [this, chassisPath, sensorPath,
         targetInterface](sdbusplus::message_t& msg) {
            try
            {
                ...
                auto it =
                    std::find(ifaces.begin(), ifaces.end(), targetInterface);
                if (it == ifaces.end())
                {
                    return;
                }
            ...
            lg2::info("ChassisManager: fan sensor {SENSOR} lost from D-Bus "
                      "(chassis {PATH}), triggering removal",
                      "SENSOR", sensorPath, "PATH", chassisPath);

            // Do NOT erase the loss watch here — we are executing inside the
            // match object's own lambda.  clearFanSensorLossWatches() called
            // by the Manager after fan removal will clean up safely.
            if (_onFanSensorLost)
            {
...
void ChassisManager::clearFanSensorLossWatches(const std::string& chassisPath)
{
    ...
    {
        lg2::debug(
            "ChassisManager: clearing {N} fan sensor loss watch(es) for {PATH}",
