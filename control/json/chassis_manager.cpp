                                     ...
                                     bool checkAvailability)
{
    ...
    _propMatches.emplace_back(
        *_bus,
        sdbusplus::match_rules::propertiesChanged(path, INV_ITEM_IFACE),
...
bool ChassisManager::isReady(const std::string& path) const
{
    if (path.empty())
    {
        ...
        // Backwards compatability for fans.json that don't have the "chassis_path" variable
        return true;
    }
    ...
    if (iface == INV_ITEM_IFACE)
    {
        ...
        if (p != props.end())
        {
            ...
            if (state.present != newVal || !state.presentIfaceSeen)
            {
                ...
                changed = true;
                lg2::info(
                    "ChassisManager: {PATH} Present changed to {VAL}",
    ...
    else if (iface == AVAILABILITY_INTF && state.checkAvailability)
    {
        ...
        if (p != props.end())
        {
            ...
            if (state.available != newVal || !state.availIfaceSeen)
            {
                ...
                changed = true;
                lg2::info(
                    "ChassisManager: {PATH} Available changed to {VAL}",
                    "PATH", path, "VAL", newVal);
            }
        }
    }
