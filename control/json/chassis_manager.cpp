        ...
        [this, chassisPath, sensorPath,
         targetInterface](sdbusplus::message_t& msg) {
            ...
            lg2::info("ChassisManager: fan sensor {SENSOR} appeared on D-Bus "
                      "(chassis {PATH}), triggering bind",
                      "SENSOR", sensorPath, "PATH", chassisPath);

            // Do NOT erase the match here — we are currently executing inside
            // the match object's own lambda.  clearFanSensorWatches() called
            // by the Manager after the fan is bound will clean up safely.
            if (_onFanSensorAppeared)
            {
                _onFanSensorAppeared(chassisPath, sensorPath, targetInterface);
            }
        });
}
