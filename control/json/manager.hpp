...
class Manager
{
  public:
    ...
    /**
     * @brief Initialise the ChassisManager and register chassis paths from
     * fans.json.
     *
     * Constructs a fresh ChassisManager, wires up all callbacks, scans
     * fans.json for chassis_path entries, and calls registerChassis() for
     * each unique path found.  Must be called before getConfig<Fan>().
     */
     ...
     */
    void handleFanSensorAppeared(const std::string& chassisPath,
     ...
     */
    void handleFanSensorLost(const std::string& chassisPath,
                             const std::string& sensorPath,
