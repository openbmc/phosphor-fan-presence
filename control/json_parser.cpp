...
const bool checkEntry(const std::vector<std::string>& activeProfiles,
        ...
        if (checkEntry(activeProfiles, zone.second->getProfiles()))
        {
            ...
            else
            {
                zoneDefs.emplace_back(std::make_tuple(
                    ...
                    zone.second->getDecInterval(),
                    zone.second->getZoneHandlers(), std::move(fanDefs),
                    std::move(speedEvents)));
            }
        }
    }
