    ...
    const std::vector<std::string>& fans, bool shortPath = false)
{
    ...
    for (auto& path : paths)
    {
        for (auto& fan : fans)
        {
            if (shortPath)
            {
                // Fan names in fanNames are derived from tach sensor D-Bus
                // paths (e.g. ".../sensors/fan_tach/chassis1_fan0_0") by
                // taking the last path component and stripping the trailing
                // "_N" rotor suffix, yielding "chassis1_fan0".  The
                // corresponding inventory paths are rooted under a per-chassis
                // sub-tree, e.g. ".../inventory/system/chassis1/fan0", so the
                // last component alone ("fan0") does not match the fan token.
                // Split on "_fan" to match both the chassis directory segment
                // and the fan leaf independently.
