...
/**
 * @function gets the states of phospho-fanctl. Does an
 * equivalent to systemctl status phosphor-fan-control@0
 */
std::array<std::string, 6> getStates()
{
    ...
    try
    {
        ...
        auto x{SDBusPlus::callMethodAndRead<std::vector<DBusTuple>>(
            systemdService, systemdPath, systemdMgrIface, "ListUnitsByNames",
            services)};

        for (auto field : x)
        {
            ret[0] = std::get<2>(field);
            ret[1] = std::get<3>(field);
            ret[2] = std::get<4>(field);
        }
    }
