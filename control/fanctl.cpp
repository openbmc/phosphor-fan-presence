...
int main(int argc, char* argv[])
{
    ...
    app.add_option("action", action, "action to perform [status|get|set|reload|resume]");
...
void loadVars()
{
    interfaces["FanSpeed"] = "xyz.openbmc_project.Control.FanSpeed";
    interfaces["FanPwm"] = "xyz.openbmc_project.Control.FanPwm";
    interfaces["SensorValue"] = "xyz.openbmc_project.Sensor.Value";
    interfaces["Item"] = "xyz.openbmc_project.Inventory.Item";
    interfaces["OpStatus"] =
...
std::array<std::string, 6> getStates()
{
    ...
    try
    {
        ...
        {
            std::cerr << "No units found for systemd service: " << services[0]
                      << std::endl;
        }
    }
...
void status()
{
    ...
    // print the header
    cerr << "Fan Control Service State   : " << states[0] << ", " << states[1]
         << "(" << states[2] << ")" << endl;
    cerr << "CurrentBMCState     : " << states[3] << endl;
    cerr << "CurrentPowerState   : " << states[4] << endl;
    cerr << "CurrentHostState    : " << states[5] << endl;
    cerr << endl;
    cerr << " FAN        "
         << "TARGET(" << tmethod << ")  FEEDBACKS(";
    cerr << fmethod << ")   PRESENT"
         << "     FUNCTIONAL" << endl;
    cerr << "==============================================================="
         << endl;
    ...
    for (auto& fan : fanNames)
    {
        ...
        // get the target RPM
        property = "Target";
        cerr << SDBusPlus::getProperty<uint64_t>(
        ...
        {
            cerr << SDBusPlus::getProperty<double>(
...
void get()
{
    ...
    // print the header
    cerr << "TARGET SENSOR" << setw(11) << "TARGET(" << tmethod
         << ")   FEEDBACK SENSOR   ";
    cerr << "FEEDBACK(" << fmethod << ")" << endl;
    cerr << "==============================================================="
         << endl;
    ...
    for (auto& fan : fanNames)
    {
        ...
        {
            cerr << setw(indent);
            cerr << path.substr(path.rfind("/") + 1) << setw(17)
...
void set(uint64_t target, std::string fans)
{
    ...
    for (auto& fan : fanList)
    {
        try
        {
            ...
            {
                std::cerr << "Could not find tach path for fan: " << fan
                          << std::endl;
                continue;
            }
