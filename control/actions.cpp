...
void set_request_speed_base_with_max(control::Zone& zone, const Group& group)
{
    ...
    std::for_each(group.begin(), group.end(),
                  [&zone, &base](auto const& entry) {
                      ...
                      {
                          // Property value not found, base request speed
                          // unchanged
                      }
                  });
