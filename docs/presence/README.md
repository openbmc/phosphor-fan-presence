...
If more then one string is found from D-Bus, the `phosphor-fan-presence-tach`
...
most specific to most general on each config file until it is found. If the
configs can't be found, it will fall back to using the `Inventory.Item`
interface.
