...
On multi-chassis systems, each fan entry carries a `chassis_path` so that
fan control waits for the sled to become present (and optionally available)
before binding its sensors.
...
`Available` are `true`. Requires `chassis_path` to be set. Optional; defaults
to `false`.
